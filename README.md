...
**Academics** — a **grade book** covering grades 5 to 12: every subject, all four
quarters and the final year mark. Pick a year from the pills, type into the grid, and it
saves as you go. Marks accept a letter (`A-`), a percentage (`88`) or a rubric score
(`3.5`) — anything at or below 4.3 is read as grade points, higher as a percentage.
Quarter and year GPAs compute themselves.

Also here: courses with their rigor level, test scores (best normalised score counts),
and an optional **official transcript GPA** for the weighted figure your school reports,
which the grade book cannot derive because weighting is school-specific. When present it
takes priority in the readiness score.

> **Only grades 9-12 feed the readiness score.** Middle school is tracked for her own
> record, but no college sees it, so letting it move a college-readiness number would
> make that number lie.
...
|---|---|---|
| Academics | 30 | 4.3 GPA — official weighted if entered, else the grade book's 9-12 average (16) · 8 rigor courses (7) · top test score (7) |
